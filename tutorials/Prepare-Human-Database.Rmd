...
```{r}
gtf <- as.data.frame(rtracklayer::import("../inst/extdata/GENCODE/gencode.v47.annotation.gtf.gz"))
table(gtf$type)
...
HumanTFDB portal: https://guolab.wchscu.cn/AnimalTFDB4/#/tf_summary/Homo_sapiens

**下载数据**

手动下载链接： https://guolab.wchscu.cn/AnimalTFDB4_static/download/TF_list_final/Homo_sapiens_TF
...
```{r}
TF <- read.delim("../inst/extdata/AnimalTFDBv4/Homo_sapiens/Homo_sapiens_TF.txt",stringsAsFactors = FALSE)
head(TF)
...
```{r}
TF <- read.delim("../inst/extdata/AnimalTFDBv4/Homo_sapiens/Homo_sapiens_Cof.txt",stringsAsFactors = FALSE)
head(TF)
TF <- TF[,c("Ensembl", "Entrez_ID", "Symbol" , "Family")]
```
...
## The IUPHAR/BPS Guide to PHARMACOLOGY - Release 2025.04.01
...
all(ligand.ID.mapping$Ensembl.ID == ligand$Ensembl.ID) # ligand.ID.mapping 与ligand 里的Ensembl.ID是一样的，并且维度也一样。
...
# 一个Ligand id 对应多个Ensembl id，1. 替换分隔符|为;，2.并且去除非人基因
tmp <- c()
...
ensembl.ids.unique.lost <- ensembl.ids.unique[!ensembl.ids.unique %in% gtf$Ensembl] # 提取不能被识别的，共计16条
...
grch37.matched.ids <- ensembl.ids.unique.lost[ensembl.ids.unique.lost %in% grch37$gene_id] # 能被grch37 ID识别的基因，共计8个
...
```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/proteinatlas_bb41022d.tsv",stringsAsFactors = FALSE)
...
table(HPA$HPA.Ensembl %in% gtf$Ensembl) # total 27 entries will be removed
HPA <- HPA[HPA$HPA.Ensembl %in% gtf$Ensembl,]
colnames(HPA)[1] <- "Ensembl"

HPA_Human_AllGenes_GO_Disease_Distribution_Location <- HPA
usethis::use_data(HPA_Human_AllGenes_GO_Disease_Distribution_Location, overwrite = TRUE)
```


**Secreted And FDA Approved Drug Targets**

url: https://www.proteinatlas.org/humanproteome/tissue/secretome

also, you can found the predicted proteins by each method in this page.

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_FDA.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")

table(HPA$HPA.Ensembl %in% gtf$Ensembl)
colnames(HPA)[1] <- "Ensembl"

HPA_Human_FDA_Approved_Secreted_Genes <- HPA
usethis::use_data(HPA_Human_FDA_Approved_Secreted_Genes, overwrite = TRUE)
```
### Protein classes
...
```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Predicted_intracellular_proteins.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")

table(HPA$HPA.Ensembl %in% gtf$Ensembl) # total 21 entries will be removed
HPA <- HPA[HPA$HPA.Ensembl %in% gtf$Ensembl,]
colnames(HPA)[1] <- "Ensembl"

HPA_Human_intracellular_Genes <- HPA
usethis::use_data(HPA_Human_intracellular_Genes, overwrite = TRUE)
```

**Predicted secreted proteins**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Predicted_secreted_proteins.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")

table(HPA$HPA.Ensembl %in% gtf$Ensembl) # total 1 entries will be removed
HPA <- HPA[HPA$HPA.Ensembl %in% gtf$Ensembl,]
colnames(HPA)[1] <- "Ensembl"

HPA_Human_Predicted_Secreted_Genes <- HPA
usethis::use_data(HPA_Human_Predicted_Secreted_Genes, overwrite = TRUE)
```
**Predicted membrane proteins**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Predicted_membrane_proteins.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")

table(HPA$HPA.Ensembl %in% gtf$Ensembl) # total 5 entries will be removed
HPA <- HPA[HPA$HPA.Ensembl %in% gtf$Ensembl,]
colnames(HPA)[1] <- "Ensembl"

HPA_Human_Predicted_membrane_Genes <- HPA
usethis::use_data(HPA_Human_Predicted_membrane_Genes, overwrite = TRUE)
```
**FDA approved drug targets**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_FDA_approved_drug_targets.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_FDA_approved_drug_target_genes <- HPA
usethis::use_data(HPA_Human_FDA_approved_drug_target_genes, overwrite = TRUE)
```
**CD markers**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_CDmarkers.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_CDmarker_Genes <- HPA
usethis::use_data(HPA_Human_CDmarker_Genes, overwrite = TRUE)
```
**GPCRs**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_G-protein.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_GPCRs_Genes <- HPA
usethis::use_data(HPA_Human_GPCRs_Genes, overwrite = TRUE)
```
**Plasma proteins**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Plasma.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_Plasma_Genes <- HPA
usethis::use_data(HPA_Human_Plasma_Genes, overwrite = TRUE)
```

**Transporters**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Transporters.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_Transporter_Genes <- HPA
usethis::use_data(HPA_Human_Transporter_Genes, overwrite = TRUE)
```
**Voltage-gated ion channels**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Voltage-gated.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")
...
HPA_Human_Voltage_Gated_Ion_Channel_Genes <- HPA
usethis::use_data(HPA_Human_Voltage_Gated_Ion_Channel_Genes, overwrite = TRUE)
```

**Transcription Factors**

```{r}
HPA <- read.delim("../inst/extdata/THE-HUMAN-PROTEIN-ATLAS/protein_class_Transcription.tsv",stringsAsFactors = FALSE)
HPA <- HPA[,c("Ensembl","Gene","Uniprot")]
colnames(HPA) <- paste("HPA.",colnames(HPA),sep = "")

table(HPA$HPA.Ensembl %in% gtf$Ensembl)

HPA_Human_ProteinClass_TranscriptionFactors <- HPA
usethis::use_data(HPA_Human_ProteinClass_TranscriptionFactors, overwrite = TRUE)
```
...
UniProt_ProteinFamily_AllSpecies <- Reduce(rbind, UniProt_ProteinFamily_AllSpecies)
table(UniProt_ProteinFamily_AllSpecies$family)
...
protein.human$Ensembl.Latest <- gtf$Ensembl[match(protein.human$Protein, gtf$UniProt)]
protein.human$Symbol.Latest <- gtf$Symbol[match(protein.human$Protein, gtf$UniProt)]
```
...
```{r}
load("../inst/extdata/CellChatV2/CellChatDB.human.rda")
...
interactions$receptor.Ensembl.Latest <- update_IDs(old = interactions$receptor.symbol, db = gtf, from = "Symbol", to = "Ensembl", split = ",", fixed = TRUE)
interactions$ligand.Ensembl.Latest <- update_IDs(old = interactions$ligand.symbol, db = gtf, from = "Symbol", to = "Ensembl", split = ",", fixed = TRUE)
```


```{r}
CellChat_Human_Ligand_Receptors_Interactions <- interactions
usethis::use_data(CellChat_Human_Ligand_Receptors_Interactions, overwrite = TRUE)
```


```{r}
lrs <- interactions[,c("ligand.Ensembl.Latest", "receptor.Ensembl.Latest", "pathway_name", "interaction_name_2")]
colnames(lrs)[4] <- "interaction"

receptor.sub <- lrs[,c(2,3,4)]
table(duplicated(receptor.sub$receptor.Ensembl.Latest))

receptor.sub <- Reduce(rbind, lapply(split(receptor.sub, f = receptor.sub$receptor.Ensembl.Latest),function(x){
...
receptor.sub <- Reduce(rbind, lapply(split(receptor.sub, f = receptor.sub$receptor.Ensembl.Latest),function(x){
  receptors <- unlist(strsplit(x$receptor.Ensembl.Latest,split = ";"))
  x <- x[rep(1,length(receptors)),]
  x$receptor.Ensembl.Latest <- receptors
  return(x)
}))
...
table(duplicated(ligand.sub$ligand.Ensembl.Latest))

ligand.sub <- Reduce(rbind, lapply(split(ligand.sub, f = ligand.sub$ligand.Ensembl.Latest),function(x){
  x$pathway_name <- paste0(unique(x$pathway_name),collapse = ";")
  x$interaction <- paste0(unique(x$interaction),collapse = ";")
  return(x[1,])
}))
...
ligand.sub <- Reduce(rbind, lapply(split(ligand.sub, f = ligand.sub$ligand.Ensembl.Latest),function(x){
  receptors <- unlist(strsplit(x$ligand.Ensembl.Latest,split = ";"))
  x <- x[rep(1,length(receptors)),]
  x$ligand.Ensembl.Latest <- receptors
  return(x)
}))
...
然后使用R语言提取基因信息，具体流程请见："inst/extdata/NCBI/Human_Gene_Summary/extract-gene-summary.md"文件 or 'prepare-data.R'。

```{r }
load("../data/GENCODE_Human_Genes.rda")
```

# 问题是： 很多基因不与GENCODE有overlap
...
```{r}
uniprot <- read.delim("../inst/extdata/UniProt/GeneFunction/Human/uniprotkb_Human_AND_reviewed_true_AND_m_2025_04_15.tsv.gz",stringsAsFactors = FALSE)
...
```{r}
uniprot$Ensembl.Latest <- GENCODE_Human_Genes$Ensembl[match(uniprot$Primary.Gene.Names, GENCODE_Human_Genes$Symbol)]
...
GENEONTOLOGY_Human <- results
load("../data/GENCODE_Human_Genes.rda")
table(GENEONTOLOGY_Human$Symbol %in% GENCODE_Human_Genes$Symbol)
GENEONTOLOGY_Human$Ensembl.Latest <- GENCODE_Human_Genes$Ensembl[match(GENEONTOLOGY_Human$Symbol, GENCODE_Human_Genes$Symbol)]
