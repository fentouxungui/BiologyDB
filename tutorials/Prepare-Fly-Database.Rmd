...
```{r}
df <- as.data.frame(readr::read_tsv("../inst/extdata/Flybase/Summary/best_gene_summary_fb_2025_01.tsv.gz",skip = 8))
head(df)
colnames(df)[1] <- 'FBgn_ID'
...
```{r}
df <- as.data.frame(readr::read_tsv("../inst/extdata/Flybase/Summary/gene_snapshots_fb_2025_01.tsv.gz",skip = 4))
head(df)
colnames(df)[1] <- 'FBgn_ID'
table(df$FBgn_ID %in% gtf$Ensembl)
...
```
```{r}
df <- as.data.frame(readr::read_tsv("../inst/extdata/Flybase/Summary/automated_gene_summaries.tsv.gz", skip = 1))
head(df)
colnames(df) <- c('FBgn_ID', 'FlybaseAutoSummary')
table(df$FBgn_ID %in% gtf$Ensembl)
...
results$Entrez_latest <- gtf$EntrezID[match(results$Ensembl, gtf$Ensembl)]
results <- results[, c("Ensembl", 'Entrez_ID', "Entrez_latest", "Symbol", "Family")]
