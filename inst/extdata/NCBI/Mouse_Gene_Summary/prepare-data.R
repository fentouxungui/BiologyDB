...
df <- read.delim("./Mus_musculus.gene_info.gz",stringsAsFactors = FALSE)
...
df$MGI <- unlist(lapply(strsplit(df$dbXrefs, split = "\\|"),function(x){
  return(paste0(gsub("^MGI:", "", x[grepl("^MGI:", x)]),collapse = ";"))
}))

results$Ensembl <- df$Ensembl[match(results$gene_id, df$GeneID)]
results$MGI <- df$MGI[match(results$gene_id, df$GeneID)]

library(AnnotationDbi)
library(org.Mm.eg.db)
library(BiologyDB)
results$Ensembl.By.OrgHSegdb <- mapIds(x = org.Mm.eg.db, keys = results$gene_id,column = "ENSEMBL",keytype = "ENTREZID",multiVals = "first")
